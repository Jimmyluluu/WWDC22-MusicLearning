    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                }.padding(/*@START_MENU_TOKEN@*/.all/*@END_MENU_TOKEN@*/).navigationBarBackButtonHidden(true).opacity(alphaValue).simultaneousGesture(TapGesture().onEnded{
                    let url = URL(fileURLWithPath: path!)
                    do {
                        audioPlayer = try AVAudioPlayer(contentsOf: url)
                        audioPlayer?.stop()}
